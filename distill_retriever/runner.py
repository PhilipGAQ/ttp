...
class EncoderOnlyEmbedderDistillRunner(AbsEmbedderRunner):
    """Runner for encoder-only query rewrite distillation."""
    ...
    def __init__(
        ...
        self.model_args: EncoderOnlyEmbedderDistillModelArguments
        self.data_args: EncoderOnlyEmbedderDistillDataArguments
        self.training_args: EncoderOnlyEmbedderDistillTrainingArguments
    ...
    def load_tokenizer_and_model(self) -> Tuple[PreTrainedTokenizer, AbsEmbedderModel]:
        ...
        model = BiEncoderOnlyEmbedderDistillModel(
            base_model,
            tokenizer=tokenizer,
            ...
            query_pos_loss_weight=self.training_args.query_pos_loss_weight,
            query_gen_pos_loss_weight=self.training_args.query_gen_pos_loss_weight,
            query_query_gen_loss_weight=self.training_args.query_query_gen_loss_weight,
            mrl_dim=self.model_args.mrl_dim,
        )
        ...
        if self.training_args.fix_position_embedding:
            for k, v in model.named_parameters():
                if "position_embeddings" in k:
                    logger.info("Freeze the parameters for %s", k)
    ...
    def load_train_dataset(self):
        if self.data_args.same_dataset_within_batch:
            train_dataset = EncoderOnlyEmbedderDistillSameDatasetTrainDataset(
                args=self.data_args,
            ...
            train_dataset = EncoderOnlyEmbedderDistillTrainDataset(
                args=self.data_args,
                tokenizer=self.tokenizer,
            )
    ...
    def load_data_collator(self):
        if self.data_args.same_dataset_within_batch:
            collator_cls = EncoderOnlyEmbedderDistillSameDatasetCollator
        else:
            collator_cls = EncoderOnlyEmbedderDistillCollator
    ...
    def load_trainer(self) -> EncoderOnlyEmbedderDistillTrainer:
        trainer = EncoderOnlyEmbedderDistillTrainer(
            model=self.model,
